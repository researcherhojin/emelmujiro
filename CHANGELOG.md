...
- E2E: local Playwright runs pin `workers: 2` + `retries: 1` (CI stays at 1 worker / 2 retries). The suite runs against the Vite dev server, which transforms modules on demand and starts dropping lazy-chunk requests under parallel load — the browser reports `TypeError: Importing a module script failed`, React's `lazy()` rejects, and the ErrorBoundary renders a `fixed inset-0 z-50` overlay, so the _visible_ failure is a click timing out on "subtree intercepts pointer events" rather than an import error. That is why it read as an unrelated WebKit flake. Measured on 18 cores: Playwright's local default (~9 workers) and 4 workers both flake, 2 and 1 are stable. The root cause is recorded in a `playwright.config.ts` comment so the next person does not re-derive it. Serving the built output instead of the dev server removes this failure class entirely, but surfaces the duplicate-head-tag bug tracked in #385, so it is deliberately not done here
...
| HTTP / network  | axios 1.15.0 → 1.17.0 (#304, #327), 1.18.0 → 1.18.1 (#353), urllib3 2.6.3 → 2.7.0 (CVE patch, backend)                                                                                                                                                                                                                                                       |
| Telemetry       | @sentry/react 10.48.0 → 10.59.0 (#271, #299, #338)                                                                                                                                                                                                                                                                                                           |
| Lint            | eslint 10.3.0 → 10.4.1 (#302), @typescript-eslint/parser 8.58.2 → 8.61.0, @typescript-eslint/eslint-plugin 8.61.1 → 8.62.0 (#349), eslint-plugin-react-hooks 7.0.1 → 7.1.1, globals 17.6.0 → 17.7.0 (#351), lint-staged 15.5.2 → 17.0.7 (#292)                                                                                                               |
...
| CI actions      | aquasecurity/trivy-action 0.35.0 → 0.36.0, actions/checkout 6 → 7 (#335), actions/cache 5 → 6 (#347, composite action aligned in `3b03acdd`)                                                                                                                                                                                                                 |
| Icons           | lucide-react 1.17.0 → 1.18.0 (#328), 1.21.0 → 1.22.0 (#350)                                                                                                                                                                                                                                                                                                  |
| Tooling         | knip 6.17.2 → 6.23.0 (#348), concurrently 9.2.3 → 10.0.3 (#354, major — `-n`/`-c` only, Node ≥24 ≥ 22, CLI so ESM-only is moot)                                                                                                                                                                                                                              |
