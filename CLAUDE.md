...
**Size**: the 2026-06 restructure set a `< 200 lines / 25 KB` target per file. Lines are within it; bytes are not (currently ~32 KB, `Gotchas` being ~11 KB of that). Two separate points, so as not to conflate them: (a) **placement** — `Gotchas` is cross-cutting, so moving it to a path-scoped rule file would mean it never auto-loads, which is a functional loss, not a formatting one; (b) **compression** — entries are dense rather than padded (#17's exact error strings, its "CI is green because the webhook `200`s on trigger-accept" note, and its health-commit check are what caught a silently blocked deploy on 2026-07-30), but no systematic pass at tightening wording in place has been done, so the overage is not itself justified. Trim wording if you want the bytes back; don't relocate the section to get them.
...
**README drift gates**: Two CI guards, both auto-healed on main by the `Sync README Badges` job in `main-ci-cd.yml` (pushes `[skip ci]`). (1) **Package badges** — 14 badges read-checked in `pr-checks.yml` vs `package.json` (red is informational only — main has no required-status-check — but visible in PRs). Auto-synced by `scripts/sync-readme-badges.sh`: dependabot patch/minor auto-merges bump `package.json` but not the badge, so this re-writes the badge and pushes, keeping unrelated open PRs from inheriting stale-badge red. The script anchors every replacement on the `/badge/<Name>-` URL prefix because label `i18next` is a substring of `React_i18next` — an unanchored sed corrupts the sibling. (2) **Test counts** — same job sed-replaces counts using `frontend-test`/`backend-test` job outputs. `make update-test-counts` is the local equivalent for counts — exits non-zero if README patterns missing. Format coupled to the sed: `Vitest (N tests) + Django unittest (N tests)` in the `**Tests** —` bullet; if you change that substring, also update `scripts/update-test-counts.sh:48,50`. **Do NOT grep test files for Vitest counts** — `it.each([...])` rows expand at runtime, so grep undercounts.
