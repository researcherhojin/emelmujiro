...
13. **`npm install` on macOS only writes the host platform's `@rolldown/binding-*` to `package-lock.json`** — npm bug #4828: regenerating the lockfile on Apple Silicon yields a single `node_modules/@rolldown/binding-darwin-arm64` entry under `packages.*`, even though rolldown's `optionalDependencies` lists 15 platforms. CI on `ubuntu-24.04` then fails with `Cannot find module '@rolldown/binding-linux-x64-gnu'` for both `Frontend Build` (vite 8 uses rolldown) and `Frontend Tests` (vitest shares the same vite). **Fix when regenerating**: `rm -rf node_modules frontend/node_modules package-lock.json && npm install --include=optional --legacy-peer-deps` — verify with `grep -c "node_modules/@rolldown/binding-" package-lock.json` (must be 15, not 1). The `--legacy-peer-deps` is needed because `eslint-plugin-jsx-a11y@6.10.2` peerDeps cap at `eslint@^9` while we run `eslint@^10` (npm@11 default install lets this slide as a warning, but a fresh `--include=optional` resolution upgrades it to ERESOLVE). Caught 2026-05-04 in `f289544`.
14. **Mixed `^3.22.x` ranges across `@tiptap/*` deps duplicate `@tiptap/core`** — when sibling tiptap packages declare different patch ranges (e.g. some `^3.22.3`, some `^3.22.5`), npm resolves multiple `@tiptap/core` versions in the tree (`3.22.4` and `3.22.5`). TypeScript treats the two `Node`/`Mark` exports as **distinct nominal types**, and `tsc -p tsconfig.build.json` fails with hundreds of lines of "`Type 'Node<any, any>' is not assignable to type 'Node<any, any>'. Two different types with this name exist, but they are unrelated.`" Fix: align every direct `@tiptap/*` dep to the same `^3.22.X` in `frontend/package.json` AND add `"@tiptap/core": "^3.22.X"` to `overrides` in both root and frontend `package.json` so any future transitive resolution dedupes. Verify with `npm ls @tiptap/core --all | grep "@tiptap/core@" | awk '{print $NF}' | sort -u` — must be a single version line. Caught 2026-05-04 in `da9d76c`.
15. **Don't manually rebase or force-push to `dependabot/*` branches** — GitHub treats non-bot pushes to dependabot-owned branches as adversarial and **closes the PR** without a clear error in `gh pr view` (the `head_sha` flips to `null` mergeable state, then state goes to `closed merged=false`). `gh pr checks` still shows the old run as if everything is fine. The intended path: comment `@dependabot rebase` and wait for the bot to push the new HEAD itself; if dependabot is racing main commits and missing them, just keep requesting rebase rather than rebasing locally. If you've already triggered the close cascade, recover by bundling the bumps into a single direct-to-main commit referencing the closed PR numbers (`Closes #NNN` lines) — that's how 2026-05-04 09a47e9 landed dompurify/eslint/typescript-eslint/globals after force-pushing one rebased branch closed all 4 open PRs.
