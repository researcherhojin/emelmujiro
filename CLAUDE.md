...
**Backend constants**: `api/constants.py` centralizes `ONE_HOUR`, `ONE_DAY`, `SPAM_KEYWORDS`, `SPAM_THRESHOLD`, `is_spam()`, and cache keys (`CACHE_BLOG_CATEGORIES`, `CACHE_BLOG_POST_LIST`, `CACHE_ADMIN_STATS`). Import from here — do NOT re-define time constants or cache keys in views/middleware. `django-extensions` + `ipython` are dev-only (`uv sync --extra dev`).
...
6. **Never run `npm audit fix` (or `--force`)** — current `9 vulnerabilities (3 low, 6 moderate, 0 high)` is the expected state. The 6 moderate are `@lhci/cli` transitives (`brace-expansion`, `express`, `qs`, `uuid`, `ws`, `@lhci/cli`) with no production impact (nginx serves static `build/` without them). `--force` tries to downgrade `@lhci/cli` to 0.1.0 and destroys Lighthouse CI. Update direct deps manually; let dependabot handle transitives.
