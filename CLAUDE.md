...
**README drift gates**: Two CI guards, both auto-healed on main by the `Sync README Badges` job in `main-ci-cd.yml` (pushes `[skip ci]`). (1) **Package badges** — 14 badges read-checked by the `Repo Checks (informational)` job in `pr-checks.yml`. Red is visible in the PR and blocks nothing. Auto-synced by `scripts/sync-readme-badges.sh`: dependabot patch/minor auto-merges bump `package.json` but not the badge, so this re-writes the badge and pushes, keeping unrelated open PRs from inheriting stale-badge red. The script anchors every replacement on the `/badge/<Name>-` URL prefix because label `i18next` is a substring of `React_i18next` — an unanchored sed corrupts the sibling. (2) **Test counts** — same job sed-replaces counts using `frontend-test`/`backend-test` job outputs. `make update-test-counts` is the local equivalent for counts — exits non-zero if README patterns missing. Format coupled to the sed: `Vitest (N tests) + Django unittest (N tests)` in the `**Tests** —` bullet; if you change that substring, also update `scripts/update-test-counts.sh:57,59`. **Do NOT grep test files for Vitest counts** — `it.each([...])` rows expand at runtime, so grep undercounts.
