...
6. **Never run `npm audit fix` (or `--force`)** — current `7 vulnerabilities (3 low, 2 moderate, 2 high)` is the expected state, and every one is a dev/CI tool that never ships: nginx serves the static `build/`. Exact provenance, measured with `npm ls`: the 2 high are `brace-expansion` (via `eslint@10.7.0` → `minimatch@10.2.5`, DoS through unbounded expansion) and `tmp` (via `@lhci/cli`, symlink temp-write); the 2 moderate are `@lhci/cli` itself and `uuid`; the 3 low are `body-parser`, `inquirer`, and `external-editor`, all `@lhci/cli` transitives. `--force` tries to downgrade `@lhci/cli` to 0.1.0 and destroys Lighthouse CI. Update direct deps manually; let dependabot handle transitives — that is how the count came down from 9: `concurrently` `10.0.3` → `10.0.4` picked up `shell-quote@1.9.0` and cleared 2 high (`shell-quote` quadratic-parse DoS, plus `concurrently` itself). Earlier clears, for the record: `form-data` (CVE-2026-12143) via the `^4.0.6` override (`046b2cff`); `express`/`qs`/`ws` via the 2026-06 axios/@tiptap bumps; `@lhci/utils`/`js-yaml` when the lockfile was regenerated for the 2026-07 dev-tooling bumps (#359–#363).
