...
- **Contact email lockstep**: `contact@emelmujiro.com` lives in **7 files** — `frontend/src/utils/constants.ts` (`CONTACT_EMAIL` fallback), `i18n/locales/ko.json` + `en.json`, `backend/config/settings.py` (`ADMIN_EMAIL` default), `backend/api/swagger.py` (`CONTACT_EMAIL` default), `CONTRIBUTING.md`, and `frontend/public/.well-known/security.txt`. Update all 7 in lockstep. **`security.txt` is the one that bites**: it publishes the address at a well-known URL security researchers actually fetch, and it was missing from this list until 2026-08-17, so a rename would have left a wrong security contact live. An 8th occurrence is comment-only — `frontend/.env.production` sets `VITE_CONTACT_EMAIL=` (empty) with a comment naming the `constants.ts` fallback; nothing breaks if it goes stale, but fix it while you are there.
