...
Rules are enforced by systems, not documentation. Documentation is advisory (~80% compliance). Hooks, linters, and CI gates are deterministic (100%).

| Layer       | Enforcement           | Example                                                 |
| ----------- | --------------------- | ------------------------------------------------------- |
| Pre-commit  | lint-staged (Husky)   | ESLint, Black, Prettier on staged files                 |
| Type system | `tsc --noEmit`        | Catches field renames, missing props                    |
...
```
1. Rename → grep first         Before renaming any symbol, grep the entire codebase for all references
2. Delete → grep first         Before deleting any export, confirm zero remaining references
3. Assume → read first         Before using an API response, version, or spec, read the actual definition
4. Fix → test after            After every change, run the relevant test suite — don't trust logic alone
5. 2 failures → change approach  If the same fix fails twice, the approach is wrong, not the execution
6. 1 commit = 1 concern        Don't mix unrelated changes — each commit should have a single purpose
7. Verify versions exist       Don't infer version numbers from patterns — check the actual release page
8. Systems enforce rules       If a rule must be followed 100%, make it a hook or CI gate, not documentation
```
